...
static irqreturn_t arduino_irq_bottom_half(int irq, void *p)
{
    ...
    dev_info(&arduino_pdev->dev, "irq=%d bottom half executed\n", irq);
    return IRQ_HANDLED;
};
...
static irqreturn_t arduino_irq_top_half(int irq, void *p)
{
    ...
    dev_info(&arduino_pdev->dev, "irq=%d top half executed\n", irq);
...
static int arduino_probe(struct platform_device *pdev)
{
    ...
    if (arduino->irq < 0) {
        dev_err(dev, "failed tp get irq\n");
        return ret;
    }

    ret = devm_request_threaded_irq(dev, arduino->irq, arduino_irq_top_half, arduino_irq_bottom_half, IRQF_TRIGGER_NONE, dev_name(&pdev->dev), pdev);
    if (ret < 0) {
